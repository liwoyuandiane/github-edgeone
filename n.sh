...
MediaUnlockTest_Netflix() {
  local RESULT_1=$(curl ${CURL_ARGS} ${MODE} --user-agent "${UA_Browser}" -SsL --max-time 10 --tlsv1.3 "https://www.netflix.com/title/81280792" 2>&1 | grep -E 'curl:|og:video|requestCountry')

  grep -q 'curl:' <<< "$RESULT_1" && return 2

  local RESULT_2=$(curl ${CURL_ARGS} ${MODE} --user-agent "${UA_Browser}" -SsL --max-time 10 --tlsv1.3 "https://www.netflix.com/title/70143836" 2>&1 | grep -E 'curl:|og:video|requestCountry')
